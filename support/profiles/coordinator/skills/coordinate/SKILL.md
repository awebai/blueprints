...
5. **Staff and assign.** One task to one agent, acceptance criteria written into
   the task. When a task needs a role and no suitable agent is free: for an
   ephemeral worker on this team, bring up a **local** agent yourself (the
   ...
   (profile + task + context) and let them provision it. Then route the work.
...
- Local agents are yours to spin up (the `aweb-agent-instantiation` skill); global,
  registered identities are agent-resources' — request those, never mint them
  yourself.
